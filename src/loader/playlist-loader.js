...
class PlaylistLoader extends EventHandler {
      ...
      if(codecs) {
        ...
        for (let i = 0; i < codecs.length; i++) {
          ...
          if (codec.indexOf('avc1') !== -1) {
            level.videoCodec = this._avc1toavcoti(codec);
  ...
  _parseMasterPlaylistMedia(string, baseurl, type, audioCodec=null) {
  ...
  _loadsuccess(response, stats, context, networkDetails=null) {
    ...
    if (string.indexOf('#EXTM3U') === 0) {
      if (string.indexOf('#EXTINF:') > 0) {
        ...
        if (levels.length) {
          let audioTracks = this._parseMasterPlaylistMedia(string, url, 'AUDIO', levels[0].audioCodec);
